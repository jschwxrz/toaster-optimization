...
for i in range(500):
    ...
    optimums[int(utility(*optimum))] = optimums.get(int(utility(*optimum)), 0) + 1
    cleaned_optimums.append(int(utility(*optimum)))
    value_for_wait[optimum[1]] = value_for_wait.get(optimum[1], 0) + 1
    value_for_toast[optimum[0]] = value_for_toast.get(optimum[0], 0) + 1  
sorted_dict = sorted(optimums.items())

cleaned_optimums.sort()
filtered_arr = []
prev_num = None

for num in cleaned_optimums:
